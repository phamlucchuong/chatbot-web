...
const LoginModal = ({ isOpen, onClose = NO_OP, onSwitchToRegister = NO_OP }) => {
    ...
    const modalClasses = `login-modal bg-white/95 backdrop-blur-md rounded-xl w-full max-w-md p-8 relative shadow-xl z-[1000] 
                          transition-all duration-300 ${isClosing ? 'scale-95 opacity-0' : 'scale-100 opacity-100'} 
                          sm:max-w-md md:p-8 sm:p-6 sm:m-0 m-4`;
    const inputClasses = `form-input w-full p-3 border text-gray-900 border-black/10 rounded-lg text-sm bg-white/80 transition-all duration-200 
                          focus:outline-none focus:border-blue-500 focus:bg-white focus:shadow-[0_0_0_3px_rgba(59,130,246,0.1)]`;
    const btnPrimaryClasses = `btn btn-primary py-3 px-6 rounded-lg text-sm font-medium cursor-pointer transition-all duration-200 border-none text-white 
                               bg-[#282A2C] hover:-translate-y-0.5 hover:shadow-lg 
                                w-full sm:w-auto ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`;
    const btnSecondaryClasses = `btn btn-secondary py-3 px-6 rounded-lg text-sm font-medium cursor-pointer transition-all duration-200 
                                 border border-black/10 bg-white/80 text-gray-700 hover:-translate-y-0.5 hover:bg-white hover:shadow-md
                                 w-full sm:w-auto ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`;
    const overlayClasses = `modal-overlay fixed inset-0 bg-black/60 backdrop-blur-sm z-50 transition-opacity duration-300`;
    // ---------------------------------------------------
        ...
        <div className={overlayClasses} onClick={handleClose}>
            <div className="flex justify-center items-center min-h-screen p-4">
                ...
                <div className={modalClasses} onClick={(e) => e.stopPropagation()}>
                    ...
                    {/* Close Button */}
                    <button className="close-btn absolute top-4 right-4 bg-none border-none text-xl text-gray-600 cursor-pointer p-2 rounded-full transition-colors duration-200 hover:bg-black/5 hover:text-gray-900" onClick={handleClose}>
                        &times;
                    </button>

                    <h2 className="text-2xl font-semibold text-gray-900 mb-6 text-left leading-tight sm:text-2xl text-xl">Đăng nhập</h2>

                    {/* Form Quên Mật Khẩu (Forgot Password Form) */}
                    <form id="forgotPasswordForm" style={{ display: isForgotPassword ? 'block' : 'none' }}>
                        <div className="form-group mb-5">
                            <label className="form-label block mb-2 text-gray-900 text-sm font-medium">Nhập email để đặt lại mật khẩu</label>
                    ...
                    <form id="loginForm" onSubmit={handleLoginSubmit} ref={loginFormRef} style={{ display: isForgotPassword ? 'none' : 'block' }}>
                        ...
                        <div className="form-group mb-5">
                            <label htmlFor="email" className="form-label block mb-2 text-gray-900 text-sm font-medium">Email <span className="text-red-500">*</span></label>
                            <input id="email" name="email" type="email" className={inputClasses} required disabled={isLoading} />
                        </div>

                        <div className="form-group mb-5">
                            <label htmlFor="password" className="form-label block mb-2 text-gray-900 text-sm font-medium">Mật khẩu <span className="text-red-500">*</span></label>
                            <div className="password-input-wrapper relative">
                                ...
                                <button type="button" className="password-toggle absolute right-3 top-1/2 -translate-y-1/2 bg-none border-none cursor-pointer text-lg text-gray-500 p-1 transition-colors duration-200 hover:text-gray-900" onClick={togglePassword} disabled={isLoading}>
                                    {showPassword
                            ...
                            <div className="forgot-password text-right mt-2">
                                <a id="forget-pwd-btn" href="#" className="text-[#282A2C] text-sm font-medium hover:underline" onClick={showForgotPassword} disabled={isLoading}>Quên mật khẩu?</a>
                            </div>
                        </div>
                        ...
                        <div className="form-footer flex justify-between items-center mt-8">
                            <div>
                                ...
                                <a onClick={onSwitchToRegister} className="signup-link text-[#282A2C] text-sm font-bold hover:underline cursor-pointer">Đăng Ký</a>
                            </div>
